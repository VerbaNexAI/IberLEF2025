...
ENDPOINT_GET_MESSAGES_TRIAL = URL + "/{TASK}/getmessages/{TOKEN}"
ENDPOINT_SUBMIT_DECISIONS_TRIAL = URL + "/{TASK}/submit/{TOKEN}/{RUN}"
...
class Client:
    ...
    def __init__(self, token: str, number_of_runs: int, tracker: EmissionsTracker):
        ...
        self.best_model_task1 = load_model(str(BASE_DIR / "models" / "final_models" / "lightgbm_task1_final"))

        # Cargar modelos para Task 2 (Clasificación de categorías)
        self.tokenizer_task2 = AutoTokenizer.from_pretrained("dccuchile/bert-base-spanish-wwm-cased")
        self.model_task2 = AutoModel.from_pretrained("dccuchile/bert-base-spanish-wwm-cased").to(device)
        self.model_task2.eval()
        self.best_model_task2 = load_model(str(BASE_DIR / "models" / "final_models" / "lr_task2_final"))
    ...
    def get_embeddings(self, text: str, task: str):
        ...
        if task == "task1":
            ...
            inputs = self.tokenizer_task2(text, return_tensors="pt", truncation=True, padding="max_length", max_length=512)
